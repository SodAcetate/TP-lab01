...
**Переменные среды** — у консольной сессии есть куча переменных, получить значения которых можно так: `${<имя переменной>}`. Свои переменные можно создавать командой `export`. Созданные таким образом переменные действительны до конца сессии (до выхода из консоли)
